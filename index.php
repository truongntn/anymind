...
if (isset($_POST['kt_sign_in_submit'])) {
  $sql = "SELECT * FROM user, admin where user.admin=admin.admin_email and user_phone='" . str_replace(' ', '', mysqli_real_escape_string($conn, $_POST['phone'])) . "' and user_password='" . mysqli_real_escape_string($conn, $_POST['password']) . "' and reference_code='" . mysqli_real_escape_string($conn, $_GET['ref']) . "' ";
  if ($result = mysqli_query($conn, $sql)) {
    $valid = true;
    if ($result && $result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        setCookie("phone", $row["user_phone"], time() + (48 * 60 * 60));
        setCookie("name", $row["user_name"], time() + (48 * 60 * 60));
        setCookie("user", $row["user_id"], time() + (48 * 60 * 60));
        setCookie("admin", $row["admin_email"], time() + (48 * 60 * 60));
      }
      header("Location: calendar");
    } else {
      $valid = false;
          ...
          <?php if ($valid === false) { ?>
            ...
            <div class="alert alert-danger d-flex align-items-center p-5 mb-10">
              ...
              <div class="d-flex flex-column">
                ...
                <span>Falsches tel oder passcode.</span>
              </div>
            </div>
            <!--end::Alert-->
          <?php } ?>
